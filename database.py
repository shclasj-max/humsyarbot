...
class DB:
    ...
    async def _normalize_order(self, col, query_filter):
        """همه آیتم‌ها را sort کن و order را از 0 بده"""
    ...
    async def reorder_up(self, collection, doc_id, query_filter):
        try:
            ...
            if idx == 0:
                return False
            await col.update_one({'_id': items[idx]['_id']},   {'$set': {'order': idx - 1}})
            await col.update_one({'_id': items[idx-1]['_id']}, {'$set': {'order': idx}})
            return True
        except:
            return False
    ...
    async def ref_add_file(self, book_id, lang, file_id, volume=1, description=''):
        ...
        if existing:
            await self.ref_files.update_one(
                {'_id': existing['_id']},
                {'$set': {'file_id': file_id, 'description': description, 'uploaded_at': datetime.now().isoformat()}}
            )
            return str(existing['_id'])
    ...
    async def add_question(self, lesson, topic, difficulty, question, options,
                           correct, explanation, creator, auto_approve=False):
    ...
    async def add_schedule(self, stype, lesson, teacher, date, time, location,
                           notes='', group='هر دو', is_weekly=False):
    ...
    async def user_stats(self, uid):
        ...
        week_act = await self.stats_col.count_documents(
    ...
    async def content_admin_stats(self):
        ...
        return {
            ...
            'bs_total': bs_total,       'bs_video': bs_video,
            'bs_pdf': bs_pdf,           'bs_ppt': bs_ppt,
            'bs_voice': bs_voice,       'bs_note': bs_note,
            'bs_test': bs_test,
            'ref_subjects': ref_subjects, 'ref_books': ref_books,
            'ref_files': ref_files,       'ref_fa': ref_fa,
            'ref_en': ref_en,
